...
function generateUniqueIdValue() {
...
function getBaseUrl(req) {
  const configured = normalizeBaseUrl(
    process.env.PUBLIC_SITE_URL || process.env.SITE_URL || process.env.APP_URL || process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL
  );
...
function decodeState(value) {
  try {
    ...
    if (parsed && typeof parsed.nonce === "string" && parsed.nonce.length >= 16 && typeof parsed.redirect === "string") {
      return {
        nonce: parsed.nonce,
...
function getStateCookieOptions(maxAge) {
  return {
    httpOnly: true,
    maxAge,
...
function slugifyUsername(value) {
  const slug = value.toLowerCase().replace(/[^a-z0-9._-]+/g, "_").replace(/^[_-]+|[_-]+$/g, "").slice(0, 50);
...
googleAuth.get("/api/auth/google/callback", async (c) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const baseUsername = profile.email ? slugifyUsername(profile.email.split("@")[0]) : `google_${profile.id}`;
    ...
    const redirectPath = user.phoneNumber ? decodedState.redirect : `/complete-profile?redirect=${encodeURIComponent(decodedState.redirect)}`;
...
function renderCallbackPage(token, error, redirectPath) {
  if (error) {
